Param (
    [array]$Targets = ("google.com", "192.168.7.1", "8.8.8.8", "wikipedia.org"),
    $Hours = .01
    )
...
$Code = {
    ...
    While ($Count -gt 0) {   
        $Ping = Get-WmiObject Win32_PingStatus -Filter "Address = '$TestTarget'" | Select @{Label="TimeStamp";Expression={Get-Date}},@{Label="Source";Expression={ $_.__Server }},@{Label="Destination";Expression={ $_.Address }},IPv4Address,@{Label="Status";Expression={ If ($_.StatusCode -ne 0) {"Failed"} Else {""}}},ResponseTime
        ...
        Write-Host ($Ping | Select TimeStamp,Source,Destination,IPv4Address,Status,ResponseTime | Format-Table -AutoSize | Out-String)
        $Count --
